...
class DocumentAPI:
    def __init__(self):
        # 固定的API密钥
        self.api_key = "app-P6KF8MQ795fLV9ueHEa8Pf5s"
            ...
            try:
                ...
                if not query_text:
                    return jsonify({
                        'success': False,
                        'error': '缺少查询文本参数 query_text',
                        'message': '请输入要查询的文档关键词'
                    }), 400

                print(f"📄 文档查询API调用: {query_text}")
                ...
                if query_result.get('success'):
                    return jsonify({
                        ...
                        'message': f'找到 {query_result.get("count", 0)} 个相关文档',
                        'timestamp': time.time()
                    }), 200
                else:
                    return jsonify({
